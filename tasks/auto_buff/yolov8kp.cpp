...
std::vector<YOLOV8KP::Object> YOLOV8KP::get_multicandidateboxes(cv::Mat & image)
{
...
std::vector<YOLOV8KP::Object> YOLOV8KP::get_onecandidatebox(cv::Mat & image)
{
  const int64 start = cv::getTickCount();  // 设置模型输入
...
float YOLOV8KP::fill_tensor_data_image(ov::Tensor & input_tensor, const cv::Mat & input_image) const
{
